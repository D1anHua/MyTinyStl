#ifndef DHSTINYSTL_SET_H_
#define DHSTINYSTL_SET_H_
...
// 异常保证:
// dhsstl::set<Key> / dhsstl::multiset<Key> 满足基本异常保证, 并对一下函数
// 做强异常安全保证:
//  * emplace
//  * emplace_hint
//  * insert
...
*/
template <typename Key, typename Compare = dhsstl::less<Key>>
class set{
public:
...
public:
    // 使用 rb_tree 定义的型别
    // 注: 我们不能通过set的迭代器改变set的元素值
    // 由于 set 键值 key 本身即参与排序, 有充当值的作用
    // 但 本身 key 不允许被修改(会改变原本的结构)
    // 所以 set 的 很多型别都是 const 的. 不允许被修改
 ...
 * @tparam Key 键值类型
 * @tparam Compare 键值比较方式, 缺省使用 dhsstl::less
*/
template <typename Key, typename Compare = dhsstl::less<Key>>
class multiset{
public:
