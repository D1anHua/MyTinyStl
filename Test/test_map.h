#ifndef DHSTINYSTL_TEST_MAP_H_
#define DHSTINYSTL_TEST_MAP_H_
...
// pair 的宏定义
#define PAIR    dhsstl::pair<int, int>
...
// ---------------------------------------------------------------------------------------------------------------------
//! @brief Test dhsstl::map
void map_test(){
    std::cout << "[=================================================================================]" << std::endl;
    std::cout << "[--------------------------- Run container test : Map ----------------------------]" << std::endl;
    std::cout << "[--------------------------- ------- API test ------- ----------------------------]" << std::endl;
    dhsstl::vector<PAIR> v;
    for (int i = 0; i < 5; ++i)
        v.push_back(PAIR(i, i));
    dhsstl::map<int, int> m1;
    dhsstl::map<int, int, dhsstl::greater<int>> m2;
    dhsstl::map<int, int> m3(v.begin(), v.end());
    dhsstl::map<int, int> m4(v.begin(), v.end());
    dhsstl::map<int, int> m5(m3);
    dhsstl::map<int, int> m6(std::move(m3));
    dhsstl::map<int, int> m7;
    m7 = m4;
    dhsstl::map<int, int> m8;
    m8 = std::move(m4);
    dhsstl::map<int, int> m9{ PAIR(1,1),PAIR(3,2),PAIR(2,3) };
    dhsstl::map<int, int> m10;
...
void multimap_test(){
    ...
    for (int i = 0; i < 5; ++i)
        v.push_back(PAIR(i, i));
    dhsstl::multimap<int, int> m1;
    dhsstl::multimap<int, int, dhsstl::greater<int>> m2;
    dhsstl::multimap<int, int> m3(v.begin(), v.end());
    dhsstl::multimap<int, int> m4(v.begin(), v.end());
    dhsstl::multimap<int, int> m5(m3);
    dhsstl::multimap<int, int> m6(std::move(m3));
    dhsstl::multimap<int, int> m7;
    m7 = m4;
    dhsstl::multimap<int, int> m8;
    m8 = std::move(m4);
    dhsstl::multimap<int, int> m9{ PAIR(1,1),PAIR(3,2),PAIR(2,3) };
    dhsstl::multimap<int, int> m10;
    m10 = { PAIR(1,1),PAIR(3,2),PAIR(2,3) };
    ...
    for (int i = 0; i < 5; ++i)
    {
        MAP_FUN_AFTER(m1, m1.insert(dhsstl::make_pair(i, i)));
    }
    MAP_FUN_AFTER(m1, m1.insert(v.begin(), v.end()));
